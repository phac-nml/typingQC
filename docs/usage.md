...
This pipeline evaluates outputs generated by `mikrokondo` for ECTyper and SISTR.
It assesses sequencing, assembly, and typing QC metrics to assign each sample an overall **PASS**, **FAIL**, or **WARNING** status and provides detailed QC messages.

For samples passing all thresholds, the pipeline generates validated metadata, including:

- **Serotypes** (SISTR and ECTyper)
- **Toxin profiles** (ECTyper)
- **STX subtypes** (ECTyper)
...
| `mikrokondo_data`               | Path to the mikrokondo-generated JSON output file for the sample (.json or .json.gz).                  |
| `predicted_identification_name` | Species predicted by mikrokondo (e.g., Salmonella, Escherichia).                                       |
| `QC_status_overall`             | Overall sequencing QC status from mikrokondo (PASSED or FAILED).                                       |
| `predicted_primary_type_name`   | Predicted serotype for the sample (from SISTR for Salmonella or ECTyper for Escherichia). Can be empty |
